...
with c1:
    with chart_panel(t("mp.stage_dist"), height=420):
        ...
        fig_stage = px.bar(
            ...
            color_discrete_sequence=[CHART_PALETTE[0]], height=340,
        )
        apply_plotly_style(fig_stage)
        fig_stage.update_layout(xaxis_title=t("mp.candidates"), yaxis_title="")
        max_val = stage_counts["count"].max()
        fig_stage.update_xaxes(range=[0, max_val * 1.1])
        st.plotly_chart(fig_stage, use_container_width=True)

with c2:
    with chart_panel(t("mp.process_status"), height=420):
...
with r1:
    with chart_panel(t("mp.rej_rate_stage"), height=420):
...
with r2:
    with chart_panel(t("mp.rej_breakdown"), height=420):
        rej_counts = df_track["rejection"].fillna("On Progress").value_counts()

        labels = [
            "Total Candidates", "On Progress", "Placement", "Ghosting", "Rejected",
        ...
        def add_flow(src, tgt, val):
            if val > 0:
                source.append(idx_map[src])
                target.append(idx_map[tgt])
                value.append(val)
        ...
        node_colors = [
            COLORS["primary"], CHART_PALETTE[1], COLORS["success"],
            COLORS["warning"], COLORS["danger"],
            COLORS["danger"], COLORS["danger"], COLORS["danger"], COLORS["danger"],
        ]

        is_dark = st.session_state.get("theme_mode", "light") == "dark"
        link_color = "rgba(255, 255, 255, 0.15)" if is_dark else "rgba(100, 116, 139, 0.25)"
...
with g1:
    with chart_panel(t("mp.ghosting_proportion"), height=400):
...
with g2:
    with chart_panel(t("mp.fu_severity"), height=400):
        if not df_ghost.empty:
            def get_granular_severity(row):
                ...
                if g == "labeled":
                    ...
                    return severity_map.get(g, "Unknown")
                return "Unknown"
            ...
            fig_sev = px.bar(
                ...
                height=320, text_auto=True,
            )
            apply_plotly_style(fig_sev)
            fig_sev.update_layout(
                xaxis_title="", yaxis_title=t("mp.cases"), showlegend=False,
            )
...
# unified master table with drill-down
with table_panel(t("mp.unified_table"), height=None):
    if df_all_ghosting.empty:
        ...
        if selected_rows:
            ...
            # load student context (cached in session)
            if "df_student_context" not in st.session_state:
                ...
                st.session_state["df_student_context"] = df_student.merge(
                    df_status, on="NIM", how="inner", suffixes=("", "_status"),
                )
            ...
            df_full_history = df_track.merge(
                df_company[["id_tracking_company", "send_date"]],
                on="id_tracking_company", how="left",
            )
            if not df_all_ghosting.empty and "id_tracking_student" in df_all_ghosting.columns:
                df_full_history = df_full_history.merge(
                    df_all_ghosting[["id_tracking_student", "ghosting_check", "days_since_update"]],
                    on="id_tracking_student", how="left",
                )
            else:
            ...
            if df_history.empty:
                ...
                for _, row in df_history.iterrows():
                    ...
                    badge_html = (
                        f'<span style="background-color: {color}; border-radius: 12px; '
                        f'padding: 4px 10px; color: white; font-size: 12px; font-weight: 600; '
                        f'white-space: nowrap;">{prog}</span>'
                    )

                    send_dt = (
                        pd.to_datetime(row["send_date"]).strftime("%Y-%m-%d")
                        if pd.notnull(row["send_date"]) else "-"
                    )
                    last_dt = (
                        pd.to_datetime(row["last_update"], format="mixed", dayfirst=True).strftime("%Y-%m-%d")
                        if pd.notnull(row["last_update"]) else "-"
                    )
                    ghost_check = row.get("ghosting_check", "-")

                    html_rows.append(f"""<tr style="border-bottom: 1px solid var(--border-color);">
    <td style="padding: 12px 16px; color: var(--text-color);">{row.get('company', '-')}</td>
    <td style="padding: 12px 16px; color: var(--text-color);">{row.get('position', '-')}</td>
    <td style="padding: 12px 16px; color: var(--text-color);">{row.get('jenis_penempatan', '-')}</td>
    <td style="padding: 12px 16px; color: var(--text-color);">{badge_html}</td>
    <td style="padding: 12px 16px; color: var(--text-color);">{send_dt}</td>
    <td style="padding: 12px 16px; color: var(--text-color);">{last_dt}</td>
    <td style="padding: 12px 16px; color: var(--text-color);">{ghost_check}</td>
</tr>""")
